...
ERA5 <- function() {
  ...
  system(paste0(
    "module load nco && ncrename -O -d valid_time,time -v valid_time,time ",
    file,
    " ",
    file
  ))
...
hindcast <- function(forecast_times, model, members = 1:9, variable = "aice") {
  members <- ifelse(
    members == "em",
    members,
    formatC(as.numeric(members), width = 2, flag = "0")
  )
  ...
  raw_files[!file.exists(raw_files)] <- NA
  raw_files
}
...
hindcast_0lag_clim <- function(models = c("S1", "S2")) {
  lapply(setNames(models, models), \(model) {
    file <- here::here("data/derived/clim_lag0_{model}.nc") |>
      glue::glue()

    # Out of gadi, we don't have the source files
    if (on_gadi()) {
      months <- 1:12
      lapply(setNames(months, months), \(month) {
        file = here::here(glue::glue(
          "data/derived/climatology/{model}/{zeropad(month)}/em.nc"
        )) |>
          rcdo::cdo_selmonth(month)
      }) |>
        rcdo::cdo_mergetime() |>
        rcdo::cdo_monmean() |>
...
hindcast_mean_thickness <- function() {
  compute_mean_thickness <- function(
    concentration,
    thickness,
    output
  ) {
    ...
    rcdo::cdo_div(volume, area) |>
      rcdo::cdo_monmean() |>
      rcdo::cdo_execute(output = output, options = "-L", cache = TRUE)
  }
  ...
  thick <- data.table::CJ(model = c("S2", "S1")) |>
    ...
    _[,
      mean_thickness := here::here(glue::glue_data(
        .SD,
        "data/derived/mean_thickness/{model}/{forecast_time}/em.nc"
      ))
    ]

  dirname(thick$mean_thickness) |>
    lapply(\(dir) dir.create(dir, showWarnings = FALSE, recursive = TRUE))
    ...
    thick <- thick |>
      ...
      _[,
        .(
          model,
          forecast_time,
          mean_thickness = unlist(mirai::mirai_map(
            data.frame(
              concentration,
              thickness = thickness,
              output = mean_thickness
            ),

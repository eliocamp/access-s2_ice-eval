...
extents <- metR::ReadNetCDF(og_extent, vars = c(og_bt = "aice")) |>
  _[
    metR::ReadNetCDF(cdr_extent, vars = c(cdr_bt = "aice")),
    on = .NATURAL
  ] |>
  # _[
...
extents |>
  ...
  _[,
    ...
    by = .(variable, yday = data.table::yday(time))
  ] |>
  # _[data.table::year(time) == 2000] |>
...
e |>
  ...
  _[,
    sqrt(mean((value2 - value)^2, na.rm = TRUE)) / 1e12,
...
(g <- metR::ReadNetCDF(abs_dif, vars = c(dif = "aice")) |>
  ...
  theme_minimal() +
  labs(
    title = "Mean absolute difference between Bootstrap from polarwatch/CDR and from NSIDC"
  ))

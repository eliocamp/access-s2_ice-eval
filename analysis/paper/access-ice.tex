...
\urlstyle{same}
\hypersetup{
  pdftitle={Evaluating the Importance of Initial Conditions for Antarctic Sea Ice Seasonal Predictability with a Fully Coupled Model},
  pdfauthor={Elio Campitelli; Ariaan Purich; Julie Arblaster; Eun-Pa Lim; Matthew Wheeler; Phillip Reid},
  pdfkeywords={sea ice, predictability, initial
...
\title[Evaluating the Importance of Initial Conditions for Antarctic Sea
Ice Seasonal Predictability with a Fully Coupled Model]{Evaluating the
Importance of Initial Conditions for Antarctic Sea Ice Seasonal
Predictability with a Fully Coupled Model}
...
that ACCESS-S2 does not assimilate sea-ice observations. This makes for
a good ``natural experiment'' to analyse the role of initial conditions
on Antarctic sea ice forecasts using a long run and a moderately large
ensemble with a fully coupled model. Our analysis reveals that both
systems experience an extended melt season and delayed growth phase.
This leads to a significant negative sea-ice extent bias which is
corrected only in ACCESS-S1 by the data assimilation system. The impact
of the initial conditions on forecast errors varies dramatically by
season: summer and autumn initial conditions (January-April) provide
predictive skill for up to three months, with February initial
conditions being particularly crucial. In contrast, winter forecasts of
the two systems are statistically indistinguishable after only two
weeks. Regional analysis of forecast skill suggests that this winter
predictability barrier is most dramatic over Eastern Antarctica, where
even ACCESS-S1 shows negative skill. These findings highlight the
critical importance of comprehensive year-round sampling in
predictability studies and suggest that operational sea-ice data
assimilation efforts should prioritize the summer-autumn period when
initial conditions have maximum impact on forecast skill.}

% keywords
\keywords{sea ice,  predictability,  initial conditions,  forecasting}
...
However, Antarctic sea ice forecasts have lagged behind Arctic sea ice
forecasts due to model biases, inherent large variability and complexity
\citep{zampieri2019, gao2024a}. Dynamical seasonal forecasts of summer
Antarctic sea ice have been shown to perform worse than relatively
simpler statistical methods \citep{massonnet2023} and machine learning
approaches (e.g. \citet{dong2024}, \citet{lin2025}), which also
...
the climate system as well as to explore the potential of using its sea
ice forecasts for decision-making. The work will also serve as a
...
are over 0ºC and Sea Surface Salinity is weakly nudged to the World
...
in this study runs for the period 1981--2018. Each hindcast consists of
nine ensemble members built from three consecutive 3-member forecasts
created in the same manner as ACCESS-S1 members initialised at the first
of every month and the two previous days and run for 279 days. We
analyse the ensemble mean hindcasts unless otherwise specified.
...
Figure~\ref{fig-hindcast-extent} shows mean sea-ice extent of the
ACCESS-S1 and ACCESS-S2 hindcasts (row a) and their differences from
...
216 days for ACCESS-S1). At this long lead time, information of the
...
growth between May and July (Figure~\ref{fig-mean-growth}). Many sea-ice
models exhibit this systematic underestimation during the sea-ice
minimum and early freezing season \citep{massonnet2023}, which could
...
and June, when the faster growth results in large positive bias along
...
Figure~\ref{fig-extent-sd} shows the interannual standard deviation of
monthly sea-ice extent of the forecasts as a function of lead time
compared with observations. ACCESS-S1 standard deviation lies within the
observed standard deviation regardless of lead time, while ACCESS-S2
standard deviation is more than twice that of observations at zero lead
time and only approaches the observed value at 9 month lead time for
most months.
...
Figure~\ref{fig-initial-spread} column 1 shows the mean standard
deviation of errors among ensemble members at various lead times. At one
day lead time (Fig.~\ref{fig-initial-spread} a.1) ACCESS-S2 has a
slightly larger spread than ACCESS-S1 due to the way that ensemble
members are generated. ACCESS-S1 ensemble members are generated by
adding random field perturbations to the atmosphere only, which then are
transferred to the other components via the coupled simulation
\citep{hudson2017}. With this scheme, ensemble members are all but
guaranteed to be underdispersed in the ocean and sea-ice components. The
time-lag ensemble used for ACCESS-S2 ensures greater spread. This
difference is gone after about just two days, and both systems have a
comparable spread in ensemble member error afterwards
...
accessed via this link https://github.com/eliocamp/access-s2\_ice-eval.

Raw data of ACCESS-S1 and ACCESS-S2 forecast are not available due to
size. Derived datasets required to reproduce the results are available
in this zenodo repository: XXXXX

...
The fact that +S1 and +S2 share the same model configuration and that the increased variability is more extreme at short lead times ([Fig. @fig-exent-sd]) suggests that the data assimilation procedure is partly to blame. 
...
```{r fig-rmse}
#| fig-cap: Mean RMSE of sea-ice concentration anomalies as a function of forecast lead time for all forecasts initialised on the first of each month compared with a reference forecast of persistence of anomalies (black) and climatology (gray). Only the first 120 days are shown.
#| fig.height: 5
#| fig.width: 9

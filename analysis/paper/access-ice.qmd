...
We analyse the ensemble mean hindcasts unless otherwise specified and, although the hindcast periods differ for each forecasting system, we only use data from the overlapping period to compute error measures. 
...
Estimates of inter-product uncertainty of sea-ice extent (SIE, defined here as the total region of the Southern Ocean with at least 15% sea-ice cover [@@cavalieri1991]) are of the order of 0.5 million $km^2$ [@meier2019].
...
+S2 forecasts of sea-ice extent anomalies seem to align moderately well with observations leading to statistically significant correlations (@fig-extent-anom, panel a.2) but their magnitude is overestimated (leading to large errors).
This could be caused by +S2 sea ice being much more sensitive to atmospheric and oceanic forcing.

As an example, @fig-forecast-example shows sea-ice concentration anomalies (top row) and sea-ice thickness and the difference between the two models (bottom row) for `r format(as.Date("2008-05-02"), "%e %B %Y")` initialised one day prior; therefore representing initial conditions. 
...
However, the magnitude of the sea-ice concentration anomalies in +S2 is too big. 
...
One possible explanation is that +S2 simulates thinner sea ice than +S1 (@fig-forecast-example bottom row) that is more sensitive to atmospheric and oceanic forcing (@fig-hi-var-cor).

...
author:
  - name: Elio Campitelli
    affiliations:
      - name: Monash University, Kulin Nations, Clayton, Victoria, Australia.
        id: 1
        ...
        # state: MA
        # postal-code: 02138
      - name: Clayton, Kulin Nations, Victoria, Australia
        id: 2
        department: SARC Special Research Initiative for Securing Antarctica’s Environmental Future
  ...
  - name: Eun-Pa Lim
    affiliations:
      - department: Research, Bureau of Meteorology, Melbourne, Australia
        id: 3
...
knitr::opts_chunk$set(
  fig.path = "access-ice_files/figure-pdf/",
  cache.path = here::here("analysis/paper/access-ice_cache/pdf/")
)
...
::: {.content-visible when-profile="main"}
...
@fig-hindcast-extent (Figure \ref{fig-hindcast-extent-osi} for OSI) shows mean sea-ice extent of the +S1 and +S2 hindcasts (row a) and their differences from mean sea-ice extent of +CDR (row b).
...
This is then followed by faster growth between May and July (@fig-mean-growth and Figure \ref{fig-mean-growth-osi}).
...
```{r fig-bias-2}
#| fig-cap: Same as Figure \ref{fig-bias-1} but for +S1.
#| fig-height: 6
...
To further understand the bias in +S2, @fig-bias-1 (Figure \ref{fig-bias-1-osi}) shows spatial patterns of the differences of monthly mean sea-ice concentrations between NOAA/NSIDC CDRV4 and +S2 hindcasts at the shortest monthly lead time.
...
+S1 has a comparatively smaller overall bias (@fig-bias-2 and Figure \ref{fig-bias-2-osi}). 
...
@fig-extent-anom (Figure \ref{fig-extent-anom-osi}) shows monthly sea-ice extent anomalies forecasted at selected lead times.
...
@fig-extent-sd (Figure \ref{fig-extent-sd-osi}) shows the interannual standard deviation of monthly sea-ice extent of the forecasts as a function of lead time compared with observations.
...
plot_forecast_example <- function(sample_date) {
  ...
  aice_forecast_example |>
    _[, model := factor(model, labels = c("S1", "S2", which_dataset))] |>
    ggplot(aes(xgrid, ygrid)) +
    geom_contour_fill(
...
Extending beyond the one case in @fig-forecast-example, @fig-mean-thickness (Figure \ref{fig-mean-thickness-osi}) shows monthly mean sea-ice thickness as a function of lead time for +S1 and +S2.
...
```{r fig-rmse}
#| fig-cap: Mean RMSE of sea-ice concentration anomalies as a function of forecast lead time for all forecasts initialised on the first of each month compared with a reference forecast of persistence of anomalies (black) and climatology (gray). Only the first 120 days are shown. In parentheses, the shortest time at which +S1 and +S2 mean RMSE is not statistically different at the 99% confidence level.
#| fig-height: 5
...
@fig-rmse (Figure \ref{fig-rmse-osi}) shows the mean RMSE of sea-ice concentration anomalies for +S1 and +S2 hindcasts compared against persistence and climatological forecasts used as a benchmark.
...
@fig-lead-time-window (Figure \ref{fig-lead-time-window-osi}) summarises the lead time window in which each hindcast is better than both the persistence forecast and the climatological forecast as a function of forecast month. 
...
rmse_lon_plots <- lapply(c("S1", "S2", "diff"), \(x) {
  if (x == "diff") {
    rmse_lon_mean_gdata2 <- rmse_lon_mean_gdata2 |>
      dcast(lag + lon + obs_dataset + month ~ version, value.var = "skill") |>
      _[, skill := S1 - S2]
    breaks <- NULL
  } else {
    rmse_lon_mean_gdata2 <- rmse_lon_mean_gdata2 |>
...
#| fig-cap: !expr glue::glue("RMSE skill score of +S1 forecasts with climatological forecast as reference computed on {uniqueN(rmse_lon_mean$lon)} meridional slices {diff(unique(rmse_lon_mean$lon))[1]}° wide as a function of lead time and longitude. Antarctica’s coastline is shown at the bottom of each panel for reference.")
#| fig-height: 7
#| dev: png
rmse_lon_plots[[1]]
```
...
#| fig-cap: Same as Figure \ref{fig-rmse_lon-1} but for +S2.
#| fig-height: 7
#| dev: png
rmse_lon_plots[[2]]
```
...
For +S1 forecasts (@fig-rmse_lon-1 and Figure \ref{fig-rmse_lon-1-osi}), skill tends to be lower off the coast of Eastern Antarctica even at short lead times; for instance, the skill score for forecasts initialised in May and June are negative between 0° and 120°E even at almost zero lead time. 
...
+S2 forecasts  (@fig-rmse_lon-2 and Figure \ref{fig-rmse_lon-2-osi}) also have lower skill over East Antarctica. 
...
#| fig-cap: Same as Figure \ref{fig-rmse_lon-1} but for the difference between +S1 and +S2.
#| fig-height: 7
#| dev: png
rmse_lon_plots[[3]]
```

Finally, @fig-rmse_lon-3 (Figure \ref{fig-rmse_lon-3-osi}) shows the difference in skill between +S1 and +S2. 
Large differences in skill indicate areas and months that are most affected by the data assimilation present in +S1.
Between January and March, which are the months in which +S1 is the most skilful (@fig-lead-time-window), most of the improvement compared with +S2 is present in the Ross and Weddell Sea. 
...
@fig-initial-spread (Figure \ref{fig-initial-spread-osi}) column 1 shows the mean standard deviation of errors among ensemble members at various lead times.
...
Forecasts initialised in winter have very little skill and +S1 and +S2 forecast errors are statistically indistinguishable after just two weeks. 
This is consistent with @libera2022's finding of a “winter predictability barrier”  in the Weddell Sea, although they describe the barrier as a sharp loss of predictability in July, and here we find a gradual reduction in skill compared with climatology around June. 
...
::: {.content-visible when-profile="supp"}

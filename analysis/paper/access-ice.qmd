...
```{r mirai_daemons}
if (!mirai::daemons_set()) invisible(daemons(6))
```
...
Getting sea-ice right might also help improve weather forecasts over and away from sea-ice regions [@rinke2006; @wang2024 @semmler2016]. 

However, producing accurate Antarctic sea-ice forecasts has been challenging due to model biases, inherent large variability and complexity, and it has lagged behind Arctic sea-ice forecasts [@zampieri2019;  @gao2024a].
...
Good initial conditions are generally required for a good forecast, however, it is not entirely known to what extent accurate sea-ice initial conditions affect the quality of the forecast and in which timescales.
Exploring seasonal predictions of Arctic sea-ice, @guemas2016 found that sea-ice initial conditions are only important in Autumn to predict Summer sea-ice, but the impact wasn't as dramatic when predicting Winter sea-ice.
@day2014 also found seasonally-varying differences in the effect of initialisation, noting that accurate Arctic sea-ice thickness lead to improved sea-ice forecast initialised in July but not when initialised in January. 

For the Antarctic, @holland2013 studied the initial-value predictability of Antarctic sea-ice in the CCSESM3 model. 
...
This is in contrast with @marchi2020, who ran perfect model experiments to argue that uncertainty in the atmosphere state and evolution is the main driver of uncertainty in Antarctic sea-ice extent prediction, with sea-ice and ocean initial conditions having lesser importance. 
...
It is hard to compare these studies since they are based on forecasts initialised at different times of the year: @holland2013 ran 20 ensemble members initialised on the 1st of January of a particular year, @marchi2020 ran forecasts from the 1st of March and 1st of September and @morioka2022 run forecasts only from the 1st of March. @marchi2020 also used a coupled ocean–sea-ice model instead of a fully coupled model like @holland2013 did. 

In October 2021 the Australian Bureau of Meteorology (BoM) upgraded the Australian Community Climate and Earth System Simulator – Seasonal (ACCES-S) to its version two. 
The change in version was focused on using ocean, sea-ice and land initial conditions generated by the BoM instead of depending on the UK’ Met Office. 
Crucially, ACCESS-S2 does not assimilate sea-ice observations, and instead sea-ice is only affected by the ocean and atmospheric data assimilation via the coupled integration.

Since model configuration is identical between the two systems, they form a sort of “natural experiment” where the same forecasting model was run over a long period of time with multiple ensemble forecasts initialised throughout the year. 
This can be an opportunity to test the effect of sea-ice initial conditions on the forecast of sea-ice concentrations. 

In this study we compare sea-ice hindcasts produced by ACCESS-S1 and ACCESS-S2. 
...
```{r range_max_lag}
range_max_lag <- hindcast_extent[mday(time) == 2, .SD[which.max(lag)], 
...
hindcast_extent[, .(model, forecast_time, member, time, Forecast = aice)] |> 
  ...
  geom_line(data = extent_climatology[dataset == which_dataset] |> 
            ...
            aes(x = as.Date(time), colour = dataset, 
                group = dataset)) + 
  
  geom_hline(data = data.table(y = 0, measure = factor("Difference", levels = c("Forecast", "Difference"))), aes(yintercept = 0)) +
  geom_line(aes(group = interaction(forecast_time, time != time2), colour = model)) +
  geom_point(data = \(x) x[mday(time) == 2, .SD[which.max(lag)], 
  ...
  facet_grid(measure ~ model, scales = "free_y", labeller = labeller(model = labels_models)) +
  tag_facets(tag = "rc") +
...
dif_hindcast <- hindcast_extent |> 
  ...
  _[, dif := c(NA, diff(aice)/diff(as.numeric(as.Date(time)))), 
    by = .(model, forecast_time)] |> 
  _[, dif := frollmean(dif, n = w, align = "center"), by = .(model, forecast_time)] |> 
  na.omit() |> 
  _[, average(dif), by = .(model, month(forecast_time), time = update(time, year = 2000))]
...
hindcast_clim <- lapply(setNames(months, months), \(month) {
  lapply(c("S1", "S2"), \(model) {
    here::here(glue::glue("data/derived/climatology/{model}/{zeropad(month)}/em.nc")) |> 
      cdo_selmonth(month) |> 
      cdo_execute() |> 
      ReadNetCDF("aice")  |> 
      _[, model := model]
...
hindcast_clim <- hindcast_clim |> 
  copy() |> 
  _[, .(aice = mean(aice)), by = .(month(time), ygrid, xgrid, forecast_month, model)] |> 
...
Strangely, for +S2, RMSE improves with lead time, even though the correlation degrades with lead time. This is puzzling behaviour that goes contrary to what is usually seen in prediction models. 
...
obs_sd <- monthly_extent[time %between% s1_range,
                                   sd_ci(aice, na.rm = TRUE),
                                   by = .(month(time), dataset)] |> 
  _[, .(dataset, month, obs_sd = estimate)]

hindcast_extent_sd <- hindast_extent_monthly[time %between% s1_range, 
...
hindcast_extent_sd |> 
  ...
  geom_ribbon(aes(ymin = low/obs_sd, ymax = high/obs_sd, fill = model), alpha = 0.5) +
  geom_line(aes(color = model)) +
  
  scale_y_continuous("Standardised sea ice extent standard deviation") +
  scale_x_continuous("Lead time", breaks = c(0:9), expand = c(0, 0), 
...
aice_forecast_example <- c(lapply(c("S1", "S2"), \(model) {
  month_clim <- formatC(month(forecast_date), width = 2, flag = "0")
  
  hindcast(forecast_date, model, 1) |> 
    cdo_ydaysub(here::here(glue::glue("data/derived/climatology/{model}/{month_clim}/em.nc"))) |>
    cdo_execute() |> 
...
thickness_example <- c(S2 = "/g/data/ux62/access-s2/hindcast/raw_model/ice/hi/daily/e01/di_hi_{format(forecast_date, '%Y%m%d')}_e01.nc",
...
aice_forecast_example |> 
  ggplot(aes(xgrid, ygrid)) +
  geom_contour_fill(aes(z = aice, fill = ..level..),
                    breaks = AnchorBreaks(binwidth = 0.1, exclude = 0)) +
  scale_fill_divergent_discretised("Sea-ice concentration anomaly",
                                   low = trans_pink,
                                   high = trans_blue) +
  geom_antarctica_fill + 
  geomcoord_antarctica +
...
thickness_example |> 
  ...
  ggplot(aes(xgrid, ygrid)) +
  geom_contour_fill(aes(z = hi, fill = ..level..), 
  ...
  scale_fill_divergent_discretised("Sea-ice thickness",
                                   high = scales::muted("#5ab4ac"),
                                   guide = guide_colorsteps(order = 9)) +
  ggnewscale::new_scale_fill() +
  
  geom_contour_fill(data = \(x) x |> 
                      dcast(time + xgrid + ygrid ~ model, value.var = "hi") |> 
                      _[, model := factor("Difference", levels = c("S1", "S2", "Difference"))], 
                    aes(z = S1 - S2, fill = ..level..), breaks = c(-Inf, b, Inf)) +
  
  scale_fill_divergent_discretised("Sea-ice thickness difference",
...
mean_thickness_access <- \(forecast_time, model) {
  concentration <- hindcast(forecast_time, model = model, members = "em", variable = "aice")
  thickness <- hindcast(forecast_time, model = model, members = "em", variable = "hi")
  
  if (length(thickness) == 0) return(NA_character_)
...
dates <- function(path) {
  ...
  list(start = lubridate::ym(date_start),
       end = lubridate::ym(date_end))
}
...
hindcast_thickness <- CJ(model = c("S2", "S1")) |>
  ...
  _[, thickness := unlist(mirai_map(data.frame(forecast_time, model), 
                                    mean_thickness_access,
                                    hindcast = hindcast, mean_thickness = mean_thickness)[.progress])] |> 
  na.omit()
...
giomas <- mean_thickness(giomas_concentration, giomas_thickness, giomas_cellarea) |> 
  cdo_ymonmean() |> 
  cdo_execute() |> 
...
hindcast_mean_thickness <- hindcast_thickness |> 
  _[, thickness |> cdo_monmean() |> cdo_execute(), by = .(model, forecast_time)] |>
  _[, metR::ReadNetCDF(V1, c(thickness = "hi")), by = .(model, forecast_time)] |> 
...
cmip |> 
  ...
  _[, mean_thickness(cdo_sellonlatbox(concentration, 0, 360, -90, -40),
    ...
    geom_line(data = giomas, aes(y = thickness, color = "GIOMAS"), linewidth = 1.4) +
...
```{r}
hindcast_mean_thickness[time %between% s1_range,
                          average(thickness), 
...
rbind(errors2 |> 
  copy() |> 
    _[obs_dataset ==  which_dataset] |> 
  _[, base := low[version == "persistence"], by = .(lag, measure, month, obs_dataset)] |> 
  _[version != "persistence"] |> 
  _[version != "climatology"] |> 
  _[estimate < base] |> 
...
errors2 |> 
  copy() |> 
  _[obs_dataset ==  which_dataset] |> 
  _[, base := low[version == "climatology"], by = .(lag, measure, month, obs_dataset)] |> 
  _[version != "climatology"] |> 
  _[version != "persistence"] |> 
  # _[lag > 2] |> 
  _[, rle(estimate < base), by= .(version, measure, month, obs_dataset)] |> 
  _[, .SD[1], by = .(version, measure, month, obs_dataset)] |> 
  tidyr::complete(version, measure, month, obs_dataset, values, fill = list(lengths = 0)) |> 
  as.data.table() |> 
  _[values == TRUE] |> 
  _[, .(version, month, obs_dataset, lag = lengths, benchmark = "climatology")] 
) |> 
  _[, time := make_date(2000, month, 1) + lag] |> 
  # _[benchmark == "climatology" & version == "S1"] |> 
  ggplot(aes(month, lag)) +
  ggbraid::geom_braid(data = \(x) dcast(x, month + lag + version ~ benchmark, value.var = "lag"),
                      alpha = 0.3,
                      aes(ymin = persistence, ymax = climatology,
                          fill = climatology > persistence)) +
  geom_line(aes(color = benchmark)) +
  # geom_point(aes(color = time), data = \(x) x[lag != 0]) +
  geom_text(data = \(x) x[, .SD[which.max(lag)], by = .(version, month, obs_dataset)] |> 
              _[benchmark == "climatology"],
            aes(label = format(time, "%d %b")),
            vjust = -0.4) +
  geom_point(data = \(x) x[, .SD[which.max(lag)], by = .(version, month, obs_dataset)] |> 
               _[benchmark == "climatology"],
             aes(color = benchmark)) +
  scale_color_models +
  scale_y_continuous("Lead time (days)") +
  scale_x_continuous("Forecast month", breaks = 1:12, labels = month.abb, minor_breaks = NULL) +
  scale_fill_manual(guide = "none", values = c(`TRUE` = "seagreen", `FALSE` = "#FFFFFF00")) +
...
gdata <- rmse_lon_mean |> 
    ...
    _[, .(dif = mean(value - climatology, na.rm = TRUE)),
      by = .(lag, lon, version, obs_dataset, month = factor(month(time_forecast)))] 
```
...
lapply(c("S1", "S2"), \(x) {
  ...
  g <- gdata |> 
    _[version == x] |> 
    _[, dif := Smooth2D(lag, lon, dif, method = smooth_svd(.1)), by = .(month, version)] |>
    ggplot(aes(lon, lag)) +
    geom_contour_fill(aes(z = dif, fill = after_stat(level)),
                      breaks = AnchorBreaks(0, binwidth = binwidth))  +
    geom_contour_tanaka(aes(z = dif), range = c(0.01, 0.2),
                        breaks = AnchorBreaks(0, binwidth = binwidth)) +
...
which_lags <- c(1, 3, 30)
rbind(mean_variance = errors_ensemble |>
        _[lag %in% which_lags] |> 
        _[measure == "rmse"] |>
        _[obs_dataset == which_dataset] |>
  ...
  scale_color_models +
  facet_grid(lag ~ component,
             labeller = labeller(component = c(mean_variance = "Mean standard deviation of errors in ensemble members",

...
The hindcast ensemble mean sea-ice extent is defined as the sea-ice extent of the ensemble mean hindcast. 
...
However, the magnitude of the sea-ice anomalies is too big. 
It is plausible that this is due to the thinner ice simulated by +S2 (bottom row). 
...
@fig-lead-time-window (@fig-lead-time-window-osi) summarises the lead time window in which each hindcast is better than both the persistence forecast and the climatological forecast as a function of forecast month. 
...
Forecasts initialised between May and July are particularly poor, and June cannot be forecasted better than the benchmarks. 
...
#| fig-cap: MSE skill score of +S1 forecasts with climatological forecast as reference computed on {uniqueN(rmse_lon_mean$lon)} meridional slices {diff(unique(rmse_lon_mean$lon))[1]}° wide as a function of lead time and longitude. Antarctica’s coastline is shown at the bottom of each panel for reference. Values were smoothed with an 11-day running mean to improve readability.
#| fig-height: 7
#| dev: png
rmse_lon_plots[[1]]
...
For +S1 forecasts (@fig-rmse_lon-1 and @fig-rmse_lon-1-osi), skill tends to be lower off the coast of Eastern Antarctica even at short lead times; for instance, the skill score for forecasts initialised in May and June are negative between 0° and 120°E even at almost zero lead time. 
...
As mentioned in @sec-systems, +S1 ensemble members are generated by adding random field perturbations to the atmosphere only, which then are transferred to the other components via the coupled simulation [@hudson2017] while +S2 ensemble members are generated by time-lagged ensemble. XXXX
...
To test the effect of the ensemble generation scheme on ensemble spread, we compute the mean variance of RMSE of individual members as a function of lag for each forecasting system. 
...
For forecasts initialised in February, March and April, +S1 even has significantly larger spread than +S2. 
XXXX What does this mean for the results??? 
...
This is consistent with @libera2022's finding of a “winter predictability barrier”, although they describe the barrier as a sharp loss of predictability in July in the Weddell Sea, and here we find a gradual reduction in skill compared with climatology around June that is more dramatic in the King Haakon Sea. 

It's not straightforward to compare our results with their, since they defined predictability based on spatial autocorraltion 

```{r}

mean_weddell <- function(x) {
  x |>
...
weddel_hk <- CDR() |>
  ...
  cdo_monmean() |>
  cdo_execute() |>
  ReadNetCDF("aice", subset = list(lon = list(c(300, 345), c(0, 120)))) |>
  _[, region := ifelse(lon > 120, "weddell", "kh")] |>
  _[year(time) %between% c(1985, 2015)]

weddell_hk_cor <- weddell_hk |>
  copy() |>
  ...
  na.omit() |>
  _[, cor(aice, value), by = .(lag, month(time), region)] |> 
  _[, measure := "sic_cor"]

list(weddell = mean_weddell, kh = mean_kh) |>
  lapply(\(x) {
    CDR() |>
      cdo_remapbil("r360x180") |>
      x() |>
  ...
  scale_fill_divergent("Sea-ice concentration correlation") +
...
This difference might be due to our use of pan-Antarctic RMSE, 
...
```{r, fig-spread-osi, ref.label=I("fig-initial-spread")}

...
Even if +S2's sea ice is more sensitive to forcing than +S1 by being overall thinner, this is not a sufficient explanation for the pattern of increased variability at shorter lead times and its absence in particular months.
Instead, the strength of the atmospheric and oceanic forcing might also be playing a role.
...
Since oceanic and atmospheric forcing provide the only source of observational information, this suggests that sea-ice in this region is particularly sensitive to oceanic and atmospheric forcing. 
...
It also suggests a role of the Pacific-South American mode and the Amundsen Sea Low to shape sea-ice concentration anomalies. 
...
Between January and March, which are the months in which +S1 is the most skillful (@fig-lead-time-window), most of the improvement compared with +S2 is present in the Ross and Weddell Seas. 
In April and May, the improvement seems more homogeneous. 
In June and July, +S1 has higher skill only at very short lead times and in limited regions; in particular we see that in June there is no improvement in the King Haakon Sea --between 0° and 90°E.
...
In both systems, correlations are relatively high around 60°S and very low in the Antarctic coast; importantly, their correlation patterns are very similar, suggesting that the initial conditions are not too dissimilar between the two systems.
A limitation of this comparison is that the low correlation seen in both systems in the ice-covered regions only show that both systems have poor ocean initial conditions in that region but does not necessarily mean that their initial conditions are similar.
...
However, since they used anomaly field assimilation, which assimilates anomalies on top of the model's own climatology, their initial conditions were close to the model's own equilibrium state, reducing the strong drift of the type we detected in +S1 and which we speculate might be degrading variabilty in +S2.
...
Contra @libera2022 but consistent with @xiu2025, we find that this predictive skill barrier is not apparent in the Weddell Sea and instead comes from low skill in the Indian Ocean sector.

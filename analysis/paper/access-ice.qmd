...
However, progress in Antarctic sea-ice forecasting has lagged behind Arctic sea-ice forecasting due to model biases and inherent large variability and complexity [@zampieri2019; @gao2024a].
...
This is particularly important for forecasts initialised between May and July, where we see that the low skill in early winter (@fig-rmse and @fig-lead-time-window) is only a feature in Eastern Antarctica where skill is even negative at almost zero lead time.
...
In both systems, correlations are relatively high around 60° and very low in the Antarctic coast; importantly, their correlation patterns are very similar, suggesting that the initial conditions are not too dissimilar between the two systems.
...
However, since they used anomaly field assimilation, which assimilates anomalies on top of the model's own climatology, their initial conditions were close to the model's own equilibrium state, reducing the strong drift of the type we detected in +S1 and which we speculate might be degrading +S2's variability.

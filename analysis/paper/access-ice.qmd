...
The +S1 hindcast is built in the same manner and running for the period 1990--2012. 

Anomalies for each hindcast are be taken with respect to a forecast-dependent climatology for the period 1990--2012. 
...
@fig-hindcast-extent a shows the median sea ice extent (left column) and median difference with respect of +CDR for the +S2 and +S1 hindcasts.
...
Both models' equilibrium state (triangles) show low bias of sea ice extent, particularly in the late-autumn and winter months.
...
It is not surprising that both forecasting systems converge to a similar climatology because they share the same model configuration. 

+S2 initial conditions (circles) also have a low extent bias, especially in the late summer-early autumn, while +S1 initial conditions are very close to observations. 
The faster growth between May and July reduces the low bias in +S2 for forecasts initialised from April to June, but it adds a high bias in +S1. 
...
```{r extent-delta-compute, fig.height=11.99/cm, fig.width=24.09/cm}
...
dif_extent |> 
  ...
  geom_smooth(aes(group = dataset, color = dataset), 
              method = "loess",
              span = 30/n, linewidth = 0.5, se = FALSE, n = n)  +
  # geom_line() +
  geom_smooth(data = dif_hindcast,
              method = "loess",
              aes(color = model), span = 30/n, linewidth = 0.5, se = FALSE, n = n) +
...
```{r fig-rmse}
#| fig-cap: Median RMSE of sea ice concentration anomalies as a function of forecast lag for all forecast initialised on the first of each month compared with a reference forecast of persistence of anomalies. 
#| fig.height: 5
#| fig.width: 9
...
```{r}
topo_lonlat <- rcdo::cdo_topo(grid = "r360x180") |>

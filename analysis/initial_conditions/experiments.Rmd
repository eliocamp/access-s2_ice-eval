...
ice_ic |> 
  ...
  wide_legend +
  labs(subtitle = "WRT NOAA CDR climatology")
```       
...
## Zonal mean zonal wind at 60ºS, 10 hPa. 
...
t <- forecast("ta", experiments) |> 
  lapply(\(x) {
    x |> 
      cdo_selname("ta") |> 
      cdo_seldate(startdate = "2024-07-01", enddate = "2024-07-15") |> 
...
t |> 
  ggplot(aes(lon, lat)) +
  geom_contour_fill(aes(z = t - 273.15)) +
  scale_fill_viridis_c() +
  wide_legend +
  coord_sf(crs = sic_projection, lims_method = "box", default_crs = "+proj=lonlat") +

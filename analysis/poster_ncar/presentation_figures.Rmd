...
```{r extent-delta, fig.height=11.99/cm, fig.width=24.09/cm}
n = 366*3
...
```{r bias_S2_0, fig.width=width_bias, fig.height=height_bias, dependson="bias_plot"}

hindcast_clim |> 
  bias_plot("S2", 0)
```

```{r bias_S1_0, fig.width=width_bias, fig.height=height_bias, dependson="bias_plot"}
...
lead
```{r bias_S2_6, fig.width=width_bias, fig.height=height_bias, dependson="bias_plot"}
hindcast_clim |> 
  bias_plot("S2", 6)
...
```{r bias_S1_6, fig.width=width_bias, fig.height=height_bias, dependson="bias_plot"}
hindcast_clim |> 
  bias_plot("S1", 6)
...
hindcast_extent |> 
  ...
  _[, lag := months_difference(time, forecast_time)] |> 
  _[, merge(.SD, monthly_extent, all = TRUE), by = .(model, lag)] |> 
  _[time %between% s1_range] |> 
  _[, .(text = sqrt(mean((aice - forecast)^2, na.rm = TRUE))), by = .(lag, model, dataset)] |> 
  ggplot(aes(lag, text)) +
...
hindcast_extent |> 
  copy() |> 
  setnames("aice", "forecast") |> 
  _[, .(forecast = mean(forecast)), by = .(time, forecast_time, lag, model)] |>
  merge(extent_daily) |> 
  _[dataset == which_dataset] |> 
  _[, clim := year(time) %between% c(1981, 2011)] |> 
  _[, let(aice = aice - mean(aice[clim == TRUE]),
          forecast = forecast - mean(forecast[clim == TRUE])),
    by = .(dataset, model, yday(time), forecast_time)] |>
  _[time %between% s1_range] |> 
  _[] |> 
  _[, .(rmse = sqrt(mean((aice - forecast)^2, na.rm = TRUE))), by = .(model, lag, dataset)] |> 
  ggplot(aes(lag, rmse)) +
  geom_line(aes(color = model)) +
...
clim_std |> 
  ...
  _[, .(value = median(climatology, na.rm = TRUE)), by = .(time = update(time, year = 2000))] |> 
...
```{r}
width_errors <- 23.87/cm
height_errors <- 11.99/cm
```


```{r rmse_persistence, fig.width=width_errors, fig.height=height_errors}
g %+% g$data[version != "climatology"]
```


```{r rmse_climatology, fig.width=width_errors, fig.height=height_errors}
...
```{r error_zoom, fig.width=width_errors, fig.height=height_errors}
p <- g +
...
```{r fig-rmse_lon, fig.height=12.6/cm, fig.width=23.1/cm}

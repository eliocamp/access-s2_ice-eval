...
mean_thickness <- \(model, month, year) {
  ...
  area <- concentration |>
    rcdo::cdo_fldint()
  out <- glue::glue(
    "data/derived/thickness/{model}/di_ice_{year}{month}01_e01.nc"
  )
...
forecast_thickness <- CJ(
  ...
  _[,
    thickness := unlist(mirai_map(
      data.frame(month, year, model),
      mean_thickness,
      hindcast = hindcast,
      cdo_del29feb = cdo_del29feb
    )[.progress])
  ] |>
  na.omit()

...
download_giomas <- function(year) {
  ...
  url <- glue::glue(
    "http://pscfiles.apl.uw.edu/zhang/Global_seaice/heff.H{year}.nc.gz"
  )
  ...
  url <- glue::glue(
    "http://pscfiles.apl.uw.edu/zhang/Global_seaice/area.H{year}.nc.gz"
  )
  ...
  list(thickness = thickness, concentration = concentration, year = year)
}

...
regions <- list(lonmins = limits[-length(limits)], lonmaxs = limits[-1]) |>
...
apply_mask <- function(file, region) {
  ...
  file |>
    cdo_expr(glue::glue(
      "\"aice = (clon(aice) >= {minlon} && clon(aice) < {maxlon}) ? aice : missval(aice)\""
    ))
}
...
compute_metrics <- function(forecast_time, member, version, obs_dataset) {
  ...
  log(glue::glue(
    "Trying to get {forecast_time} forecast for {version} member {member}"
  ))
  file <- hindcast(forecast_time, model = version, members = member)

  if (length(file) == 0) {
    stop(glue::glue(
      "No file found for model {version} at forecast time {forecast_time} member {member}"
    ))
  }

  model_climatology <- here::here(
    "data/derived/climatology",
    version,
    pad_number(month(forecast_time)),
    "em.nc"
  )

  file_template <- paste0(
    "di_aice_",
    format(forecast_time, "%Y%m%d"),
    "_e0",
    member,
    ".nc"
  )
  ...
  rmse_lon_persistence_out <- file.path(
    rmse_lon_dir,
    "persistence",
    obs_dataset,
    paste0("di_aice_", format(forecast_time, "%Y%m%d"), "_e01.nc")
  )
  dir.create(
    dirname(rmse_lon_persistence_out),
    showWarnings = FALSE,
    recursive = TRUE
  )
  ...
  rmse_out_persistence <- file.path(
    rmse_dir,
    "persistence",
    obs_dataset,
    paste0("di_aice_", format(forecast_time, "%Y%m%d"), "_e01.nc")
  ...
  if (
    all(file.exists(c(
      rmse_out_persistence,
      rmse_out,
      iiee_out,
      rmse_lon_out,
      rmse_lon_persistence_out
    )))
  ) {
    return()
  }
  ...
  log(glue::glue(
    "Computing {forecast_time} - {version} {member} with {obs_dataset}"
  ))
  ...
  observation_part_anomaly <- get_part(
    obs$anomalies[[obs_dataset]],
    first_time,
    last_time
  )
  ...
  if (!file.exists(rmse_lon_out)) {
    ...
    file |>
      cdo_ydaysub(model_climatology) |>
      cdo_rmse_lon(observation_part_anomaly, output = rmse_lon_out)
  }
  ...
  if (!file.exists(rmse_lon_persistence_out)) {
    ...
    persistence |>
      cdo_rmse_lon(observation_part_anomaly, output = rmse_lon_persistence_out)
  }

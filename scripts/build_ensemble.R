...
get_file <- function(forecast_time, member, model, variable = c("aice", "hi")) {
  ...
  if (model == "S2") {
    dir <- glue::glue("/g/data/ux62/access-s2/hindcast/raw_model/ice/{variable}/daily/e0")
...
build_ensemble <- function(forecasts, nominal_date, model, variable) {
  ...
  data <- data.table::CJ(original_forecast_date = forecasts, original_member = members) |> 
    _[, file := get_file(original_forecast_date, original_member, model, variable = variable)] |> 
    _[, member := 1:9] |> 
    _[, forecast_date := as.Date(nominal_date)] |> 
    _[, file_out := file.path(folder, paste0(formatC(member, width = 2, flag = "0"), ".nc"))] |>
    _[, process_ice(file, as.numeric(forecast_date - original_forecast_date), file_out, variable),
